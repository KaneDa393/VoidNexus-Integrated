...
function VoidNexusLib:MakeWindow(WindowConfig)
	...
	local WindowTopBarLine = AddThemeObject(SetProps(MakeElement("Frame"), {Size = UDim2.new(1, 0, 0, 1), Position = UDim2.new(0, 0, 1, -1)}), "Stroke")
		...
		local MainWindow = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 10), {
			Parent = VoidNexusGUI, Position = UDim2.new(0.5, -307, 0.5, -172), Size = UDim2.new(0, 615, 0, 344), ClipsDescendants = true, Visible = true
		}), {
		...
		function ElementFunction:AddSection(SectionConfig)
			SetChildren(SetProps(MakeElement("TFrame"), {Size = UDim2.new(1, 0, 0, 20), Parent = Container}), {AddThemeObject(SetProps(MakeElement("Label", SectionConfig.Name or "Section", 13), {Size = UDim2.new(1, 0, 1, 0), Font = Enum.Font.GothamBold}), "TextDark")})
				...
				function ElementFunction:AddButton(ButtonConfig)
					...
					AddConnection(Button:FindFirstChildOfClass("TextButton").MouseButton1Click, ButtonConfig.Callback or function() end)
				end
				...
				function ElementFunction:AddToggle(ToggleConfig)
					...
					AddConnection(Toggle:FindFirstChildOfClass("TextButton").MouseButton1Click, function() Toggled = not Toggled; Toggle.Frame.Check.Visible = Toggled; if ToggleConfig.Callback then ToggleConfig.Callback(Toggled) end end)
				end
			...
			function ElementFunction:AddTextbox(TextboxConfig)
				...
				local Input = Textbox:FindFirstChild("Frame") and Textbox.Frame:FindFirstChild("Input")
				if Input then
					AddConnection(Input.FocusLost, function() if TextboxConfig.Callback then TextboxConfig.Callback(Input.Text) end end)
				end
			end
		return ElementFunction
	end
